...
To create a user-built function, you will using the `function()` to create an R object that is a function. To use the function Inside the `funtion()` parantheses, write the arguments that need to specified for your function. 
...
The `apply()` is used to apply a function to the margins of an array or matrix. It will iterate between the elements, apply a function to the data, and return a vector, array or list if necessary. To use the `apply()`, you will need to specify three arguments, `X` or the array, `MARGIN` which margin to apply the function on, and `FUN` the function.
...
The `lapply()` is used to apply a function to all elements in a vector or list. The `lapply()` function will then return a list as the output. 

### `sapply()`

The `sapply()` is used to apply a function to all elements in a vector or list. Afterwards, the `sapply()` will return a "simplified" version of the list format. This could be a vector, matrix, or array.
